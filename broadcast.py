        ...
        try:
            ...
            if(
                ...
                if(i in groupList):
                    groupList.remove(i)
                pass
            elif(e.error_code == 403 and e.description == 'Forbidden: bot was kicked from the channel chat'):
                print(e)
                channel_handler.delete_channel(message[1])
                message_handler.delete_channel_messages(message[1])
                pass
            elif(e.error_code == 400 and e.description == 'Bad Request: group chat was upgraded to a supergroup chat'):
                print(e)
                group_handler.delete_group(i)
                if(i in groupList):
                    groupList.remove(i)
                pass
            elif(e.error_code == 403 and e.description == 'Forbidden: bot is not a member of the channel chat'):
                channel_handler.delete_channel(message[1])
                message_handler.delete_channel_messages(message[1])
                pass
            elif(e.error_code == 400 and e.description == "Bad Request: PEER_ID_INVALID"):
                group_handler.delete_group(i)
                if(i in groupList):
                    groupList.remove(i)
                pass
